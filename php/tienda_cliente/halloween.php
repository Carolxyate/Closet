...
if (isset($_SESSION['id_cliente']) && isset($_GET['id_disfraz'])) {
    ...
    if ($result->num_rows > 0) {
        $mensaje = "El disfraz ya está en tu lista de favoritos.";
    } else {
        ...
        if ($stmt->execute()) {
            $mensaje = "Disfraz agregado a favoritos correctamente.";
        } else {
            $mensaje = "Error al agregar el disfraz a favoritos: " . $conexion->error;
        }
    }
} else {
    if (isset($_GET['id_disfraz'])) {
        $mensaje = "Por favor, inicia sesión para agregar a favoritos.";
    }
}
?>
    ...
    <header>
        <nav class="navegacion">
            <ul class="menu">
                ...
                <li class="lista"><a href="favorito.php">FAVORITOS</a></li>
                <li class="buscar">
                    <form action="get">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                if (isset($_SESSION['id_cliente'])) {
                    echo '<a href="favorito.php?id_disfraz=' . $row["id_disfraz"] . '" class="card-link agregar-favoritos">Agregar a favoritos</a>';
                } else {
                    echo '<a href="inicio_sesion.html" class="card-link agregar-favoritos">Inicia sesión para agregar a favoritos</a>';
                }
